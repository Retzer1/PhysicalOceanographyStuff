function [SHEARS, numTriangles] = ShearAnimate(struc, errorQ, fps, filename) %output is a 3d matrix, each page corresponds to a timestep, holds a column vector containing shear deformation estimates for all triangles at that timestep. 
newStruc = struc;
    ...
    if triLength ~= 0
        ...
        if triLength ~= 0
            ...
            if triLength ~= 0 %If this passes, then you have prev, next, and cur triangles not deformed. 
                ...
                SHEARS(1:triLength, 1, i) = divergences; %Store divergences (shear deformations) for this datenum, rows linked to triangles
...
for i = 1:lenDate
    if numTriangles(i, 1) ~= 0 %OTherwise skip and leave it as sheet of nans
        for j = 1:numTriangles(i, 1)
            Colors(j, 1:3, i) = equiv_color(SHEARS(j, 1, i), rngArray, [0 1], cmocean('balance'));
        end
    end
end
