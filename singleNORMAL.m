function [datenums, output] = singleNORMAL(struc, drifterNum1, drifterNum2, drifterNum3, errorQ)
    datenums = find_datenum(struc);
    lenDate = length(datenums);
    output = NaN(lenDate, 1); %This will store normal deformation for each datenum for our triangle
    newStruc = struc;
