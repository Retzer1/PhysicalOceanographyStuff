function [datenums, output] = singleDivergence(struc, drifterNum1, drifterNum2, drifterNum3, errorQ)
    datenums = find_datenum(struc); %What's helpful about this def of datenums is that it's the datenum list for the entire structure, so can be used to compare any triplet from that structure along same axes without any cutting required
    %datenums = datenums(cropStartDate:end); %Crop datenums to a certain start index
