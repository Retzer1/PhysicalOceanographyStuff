function [VORTS, numTriangles] = All_Triplet_Vorticity(struc, errorQ) %output is a 3d matrix, each page corresponds to a timestep, holds a column vector containing divergence estimates for all triangles at that timestep. 
newStruc = struc;
    ...
    if triLength ~= 0
        ...
        if triLength ~= 0
            ...
            if triLength ~= 0 %If this passes, then you have prev, next, and cur triangles not deformed. 
                ...
                VORTS(1:triLength, 1, i) = divergences; %Store divergences for this datenum, rows linked to triangles
                
     

                %outdated: CALCULATE MEAN DIVERGENCE
