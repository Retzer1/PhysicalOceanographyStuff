function [output, numTriangles] = avgDivNormalizedNoSmall(struc, errorQ, meanFlag) %output is a vector of length as many time steps as we have. 
%At present, we only make sure a triangle is not deformed at its cur/prev/next time steps. Also, we assume all drifters
...
for i = 1:lenDate %Loop over datenums
    ...
    for j = 1:triLength
        ...
        if L <= errorQ || Area <= 0.1 %Discard small triangles, as well as deformed ones
