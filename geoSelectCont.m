...
function indices = geoSelectCont(struc, drifterNum) %Returns a vector of indices at which drifterNum is within the user-selected geographic square
    x = 1;
    while x == 1
        geoplot(struc(drifterNum).lat, struc(drifterNum).lon, 'o')
        pause;
        ...
        indices = cat(1, indices, find(checkIndices)); %Finds the row #s that survived, that is the indices of the data pts that survived our latest check, and appends to the current list of selected indices from previous checks
