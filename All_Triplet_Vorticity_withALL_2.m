function [VORTS, numTriangles, SIZES, DISTS] = All_Triplet_Vorticity_withALL_2(struc, errorQ, centerArray) %output is a 3d matrix, each page corresponds to a timestep, holds a column vector containing divergence estimates for all triangles at that timestep. 
%Output gives us DISTS as avg distance (not COM distance as with the previous version of this function) of each drifter in a triplet to eddy
%center at that time, centerArray identifies eddyCenter for each timestep
%of our struc structure

newStruc = struc;
%Struc must be date-selected such that lenDate has same length as centerArray (and corresponds to same dates of course) 
...
SIZES = NaN(100, 1, lenDate); %record triangle sizes, sq km
VORTS = NaN(100, 1, lenDate);
DISTS = NaN(100, 1, lenDate);
    ...
    if triLength ~= 0
        ...
        if triLength ~= 0
            ...
            if triLength ~= 0 %If this passes, then you have prev, next, and cur triangles not deformed. 
                %CALCULATE DIVERGENCES and SIZES
                ...
                sizes = triangles(:, 4); %4th column records areas of triangles
                %FIND COM LAT
                ...
                for j = 1:triLength
                    ...
                    actualLon1 = newStruc(drifterNum1).lon(index1);

                    %LAT/LON Of POINT 2
                    actualLat2 = newStruc(drifterNum2).lat(index2);
                    actualLon2 = newStruc(drifterNum2).lon(index2);

                    %LAT/LON Of POINT 3
                    actualLat3 = newStruc(drifterNum3).lat(index3);
                    actualLon3 = newStruc(drifterNum3).lon(index3);
                    
                    meanLat = (actualLat1 + actualLat2 + actualLat3)/3;
                    meanLon = (actualLon1 + actualLon2 + actualLon3)/3;
                    
                    triangles(j, 7) = meanLat;
                    %THIS IS WHERE THE PROGRAM DIFFERS FROM VERSION
                    %1-----------------------
                    d1 = geo_distance_COOK(actualLon1, actualLat1, centerArray(1, i), centerArray(2, i));
                    d2 = geo_distance_COOK(actualLon2, actualLat2, centerArray(1, i), centerArray(2, i));
                    d3 = geo_distance_COOK(actualLon3, actualLat3, centerArray(1, i), centerArray(2, i));
                    triangles(j, 8) = mean([d1 d2 d3]);
                    %------------------------------------------
                end
                ...
                dists = triangles(:, 8);
                VORTS(1:triLength, 1, i) = divergences; %Store divergences (vorticity in this case) for this datenum, rows linked to triangles
                SIZES(1:triLength, 1, i) = sizes; %Store trisizes for this datenum, rows linked to triangles
                DISTS(1:triLength, 1, i) = dists;
...
function output3 = findTriangleArea(s11, s22, s33)
