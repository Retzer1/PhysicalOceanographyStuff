function matrix_output = disp_matrix(struc)
%This calculates a matrix where each row is a drifter in a certain structure, and the (i, j) entry
%is the displacement of the j-th 12 hour segment for the i-th drifter in that structure.
...
%Now actually create the displacement matrix. 
%21 hours = 252 5-min intervals
matrix_output = zeros(length(struc), (fix((maximum-1)/252) + 1));
for i = 1:length(struc)
    for j = 0:((fix(length(struc(i).lat)-1)/252) - 1)
        
        matrix_output(i, j+1) = geo_distance_COOK(struc_lon(i, j*252 + 1),struc_lat(i, j*252 + 1),struc_lon(i, (j+1)*252 +1),struc_lat(i, (j+1)*252 +1));
    end
end
%Use below line to get the 21-hr avg velocity magnitude matrix.
matrix_output = matrix_output/75.6; %(*1000 meters per kilometer, / 252*300 seconds per 21 hours) 
end