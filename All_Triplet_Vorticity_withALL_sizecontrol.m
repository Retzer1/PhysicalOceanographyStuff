function [VORTS, numTriangles, SIZES, DISTS] = All_Triplet_Vorticity_withALL_sizecontrol(struc, errorQ, centerArray, minSize, maxSize) %output is a 3d matrix, each page corresponds to a timestep, holds a column vector containing divergence estimates for all triangles at that timestep. 
newStruc = struc; %minSize, maxSize should be 2 to 5.5 sq km
%Struc must be pre-selected such that centerArray must have length lenDate (see
%below)
...
for i = 1:lenDate %Loop over datenums
    ...
    for j = 1:triLength
        ...
        %DEFORMATION CHECK
        L = Lambda(s1, s2, s3);
        if L <= errorQ || (Area > maxSize || Area < minSize)
    ...
    if triLength ~= 0
        ...
        for j = 1:triLength
            ...
            L = Lambda(s1, s2, s3);
            if L <= errorQ || (nextArea > maxSize || nextArea < minSize)
        ...
        if triLength ~= 0
            ...
            for j = 1:triLength
                ...
                if L <= errorQ || (prevArea > maxSize || prevArea < minSize)
            ...
            if triLength ~= 0 %If this passes, then you have prev, next, and cur triangles not deformed. 
                ...
                for j = 1:triLength
                    ...
                    triangles(j, 8) = geo_distance_COOK(centerArray(1, i), centerArray(2, i), meanLon, meanLat);
                end
